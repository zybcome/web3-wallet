<template>
    <div id="box">
        ...
        <div id="right">
            <h1>MetaMask钱包</h1>
            <van-divider></van-divider>
            <div class="btn"><van-button v-if="addressMateMask == ''" type="primary"
                    @click="MetaMaskLogin()">登录</van-button>
            </div>
            <div v-if="addressMateMask != ''">
                ...
                <div v-if="transactionFlageMateMask">
                    ...
                    <p>交易明细：<span v-if="pushHerfMateMask == '加载中'">加载中</span><a target="_blank"
                            v-if="pushHerfMateMask != '加载中'" :href="pushHerfMateMask">在区块链浏览器上查看</a></p>
                </div>
            </div>
        </div>
...
const getBalanceFun = async () => {
    const accounts = await web3MateMask.eth.getAccounts().catch(err => {
        showDialog({
            message: err.message,
            theme: 'round-button',
        }).then(() => {
